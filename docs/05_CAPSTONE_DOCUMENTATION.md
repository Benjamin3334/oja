...
| 9 | Applying the baseline repeatedly failed. Early runs died inside function bodies; a later run failed at `count(distinct s.id)` in a view | Two independent causes. (a) The SQL editor splits a script client-side and mis-handles `$$`-quoted function bodies, cutting them at the semicolons inside `begin ... end;`. (b) SQL copied out of a chat transcript was being altered in transit, so the text pasted was not the text on disk | (a) Renamed every dollar quote to a named tag (`$fn_complete_sale$`) and split the baseline into numbered chunks run one at a time. (b) Stopped pasting from the transcript and pasted the generated `.sql` files directly, after verifying them byte-for-byte as pure ASCII | A failed batch rolls back and leaves a database that looks untouched, so verify the objects exist rather than trusting a success message. And when text must survive a copy, check the source bytes rather than assuming the transport is lossless |
| 10 | `supabase db push` would have applied migrations directly and avoided the editor entirely, but was deliberately not used | `db push` only recognises migrations named `<14-digit-timestamp>_name.sql`. Adopting that convention would replace the `0001`–`0008` sequence, whose numbering encodes the order in which the fixes must be applied and which is referenced throughout `01_PRD.md` | Kept the numbered filenames as the source of truth and applied the baseline through the editor in chunks. Migrations 0002 onward are small enough to paste individually | A tool convention and a documentation convention can genuinely conflict. Choose the one the reader needs, and write down why the other was rejected |
| 11 | A deactivated member landed on `/onboarding` and was refused with "This account already belongs to an organisation" | `org_select` hides the organisation once `current_org_id()` returns null, so the joined profile query returned null — **the same null as "never had an organisation"** | `getOwnProfileRow()` reads the caller's own row *without* the join, using `profiles_select_self` from 0016. A missing row means a new user; an inactive row means a removed one. Re-checked on every visit, so reactivation takes effect on reload | An empty result under RLS can mean "does not exist" **or** "you may not see it". The application must not assume which — and where it needs to tell them apart, a policy has to make the distinction readable |
