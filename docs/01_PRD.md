...
| **Database development** | A normalised (3NF) PostgreSQL schema on Supabase with primary/foreign keys, a many-to-many relationship resolved through a junction table, constraints, views, triggers and row-level security. Documented in `0001_initial_schema.sql`. |
...
| FR-2.3 | Top 5 products by revenue (last 30 days) | Matches the reference query in `0001_initial_schema.sql` §Queries |
...
Full DDL, constraints, views, triggers, policies, seed data and reference queries live in **`0001_initial_schema.sql`**. Summary here; normalisation argument in §6.3.
...
├─ supabase/migrations/        # 0001_initial_schema.sql split into ordered migrations
...
|---|---|---|
| **Mon** | Repo, Supabase project, run `0001_initial_schema.sql`, seed data, auth + app shell | You can sign in and see an empty dashboard styled with the tokens |
